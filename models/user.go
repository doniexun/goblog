...
type User struct {
	...
	QQ            string    `orm:"size(20);null"`               // QQ号
	...
	RegsiterIP    string    `orm:"size(50)"`                    // 注册 IP 地址
	LastLoginTime time.Time `orm:"type(datetime)"`              // 最后登录时间
	LastLoginIP   string    `orm:"size(50)"`                    // 最后登录 IP 地址
	LevelBonus    int       `orm:"default(0)"`                  // 等级积分
	ActiveBonus   int       `orm:"default(0)"`                  // 活跃积分
	Status        int       `orm:"default(0)"`                  // 用户账号状态：0：正常；1：异常；2：注销
	Groups        []*Group  `orm:"rel(many)"`                   // 用户所在群（若要查该用户创建的群，需要二次查询群表）
	Punchs        []*Punch  `orm:"rel(many)"`                   // 用户创建的打卡事项
}
